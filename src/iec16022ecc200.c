		  ...
		  const char *encoding, int *lenp, int flags)
{
	...
	while (sp < sl && tp < tl) {
		...
		if ( (tl - tp <= 1 && (enc == 'c' || enc == 't' || enc == 'x') ) || (tl - tp <= 2 && enc == 'e') )
...
{
    return iec16022ecc200f(Wptr, Hptr, encodingptr, barcodelen, barcode, lenp, maxp, eccp, 0, 1);
}
...
unsigned char *iec16022ecc200f(int *Wptr, int *Hptr, char **encodingptr,
			      int barcodelen, const unsigned char *barcode,
			      int *lenp, int *maxp, int *eccp, int flags, int square)
{
			...
			for (matrix = ecc200matrix; matrix->W; matrix++)
				if (ecc200encode
				    (binary, matrix->bytes, barcode, barcodelen,
				     encoding, 0, flags) && (!square || matrix->W == matrix->H) )
					break;
		} else {
			int len;
			char *e;
		...
		for (y = 0; y < NR; y++) {
			...
			for (x = 0; x < NC; x++) {
				...
				if ( v == 1 || ( v > 7 && (binary[(v >> 3) - 1] & (1 << (v & 7))) ) )
					grid[(y + border_offset_y) * W + x + border_offset_x] = 1;
			}
			//fprintf (stderr, "\n");
		}
		free(places);
